<template>
  <div id="app">
    ...
    <el-card class="box-card" v-if="tipsVisible">
      <h3 v-if="count">总计有<el-tag>{{ count }}</el-tag>条课程</h3>
      <h3 v-if="studentNotInRenGuang > 0">
        其中 <el-tag type="success">{{ count - studentNotInRenGuang }}</el-tag> 条课程在本校区上课，<el-tag type="danger"> {{ studentNotInRenGuang }}</el-tag>条课程不在半海人广,分别是
        <div class="not-in-place" v-for="(item, index) in studentNotInRenGuangNameList" :key="index"><el-tag type="danger">{{ `${item.time} - ${item.student} - ${item.subject}-${item.studentClassPlace}` }}</el-tag></div>
      </h3>
      <h3 v-if="count > 0 && studentNotInRenGuang <= 0">
        所有学生都在半海人广校区上课<i class="el-icon-success" style="color:green;"></i>
      </h3>
      <h3>
        拥有两条课程以上的学生是：
        <el-tag class="not-in-place" v-for="(item, index) in moreThanTwoClassStudentList" :key="index">{{ item[0].student }}</el-tag>
      </h3>
    </el-card>

    <el-card class="box-card2" v-if="tipsVisible">
      <el-tag type="danger"  class="not-in-place">仅！供！参！考！记！得！二！次！确！认！！！</el-tag>
      <el-tag type="info"  class="not-in-place">下载的课表根据排课网站的JSON文件，字段自己挖的，如果那边开发改了则失效哦~</el-tag>
      <el-tag type="info"  class="not-in-place">下载的excel修改为按照排课表的顺序，方便一次复制全部，并且把助教映射上去了</el-tag>
      <el-tag type="info"  class="not-in-place">统计了一下上课校区不在半海人广的学生，并给出名单</el-tag>
      <el-tag type="info"  class="not-in-place">如果学生在当天不止一节课，excel中则把他们排在一起，并给出了学生名单</el-tag>
    </el-card>
...
exportToExcel() {
      ...
      const sortedClassList = this.sortByPropertyOrder(listAfterTranslate, 'student');
      ...
      // 假设你有一个表格数据的数组
      const data = [
        ["上课时间", "科目", "老师", "学生", "主带SA", '今日SA', '上课校区', '补充说明'],
      ];
